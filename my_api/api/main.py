...
def send_email(name: str, email: str, phone: str, language: str, description: str):
    ...
    if not all([smtp_server, smtp_port_ssl, smtp_port_tls, email_user, email_password, recipient_email]):
    ...
    logging.info(f"SMTP Port SSL: {smtp_port_ssl}")
    logging.info(f"SMTP Port TLS: {smtp_port_tls}")
    ...
    try:
        ...
        logging.error(f'Failed to send email using SSL. Error: {e_ssl}')
        ...
        try:
            ...
            logging.error(f'Failed to send email using TLS. Error: {e_tls}')
            return False
